...
function renderInsertBar(container, parentId, index) {
...
function handleDrop(targetId) {
    ...
    drag.parent.children =
        drag.parent.children.filter((c) => c.id !== draggedId);
    // Insert before target
    let targetIndex = target.parent.children.findIndex((c) => c.id === targetId);
...
function renderEditor() {
    ...
    del.onclick = () => {
        result.parent.children =
            result.parent.children.filter((c) => c.id !== node.id);
        selectedId = null;
        renderPreview();
        renderEditor();
    };
...
// MODAL
// =======================
function openModal(parentId, index) {
...
function addComponent(type) {
    ...
    //Add component
    parent.children.splice(insertContext.index, 0, components.createComponent(type));
    closeModal();
    renderPreview();
}
function closeModal() {
 ...
 */
export function loadComponent(data) {
